...
export default function ExplorerBrand4gramDetail() {
  ...
  const [data, setData] = useState<BrandFourgramDetail | null>(null);
  const [err, setErr] = useState<string | null>(null);

  useEffect(() => {
    if (!fourgram) return;
    fetchApi<BrandFourgramDetail>(`/explorer/brands/4grams/${encodeURIComponent(fourgram)}`)
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      <Link to="/explorer/brands/4gram" className="text-[13px] no-underline"
            style={{ color: "var(--accent-11)" }}>
        ← 4-grams
      </Link>

      <div className="flex items-baseline gap-3 mt-2 mb-1 flex-wrap">
        <Heading size="6" className="font-mono">{data.fourgram}</Heading>
        <Badge color={data.is_distinctive ? "jade" : "gray"}>
          {data.is_distinctive ? "distinctive" : "not distinctive"}
        </Badge>
      </div>
      <Text size="2" style={{ color: "var(--gray-9)" }}>
        tokens: <strong>{data.token_a}</strong> + <strong>{data.token_b}</strong> + <strong>{data.token_c}</strong> + <strong>{data.token_d}</strong>
      </Text>
      ...
      <div className="rounded-[var(--card-radius)] border border-[var(--gray-6)] p-5">
        <div className="flex flex-wrap gap-2 items-center">
          ...
          <Text size="2" style={{ color: "var(--gray-9)", marginLeft: "auto" }}>
            distinctiveness is gated on the 4-gram's own IDF
          </Text>
        </div>
      </div>
