...
import type { BrandBigramListResponse, BrandBigramSummary } from "../types";
import ExplorerTabs from "../components/explorer/ExplorerTabs";
import BrandSearchBar from "../components/explorer/BrandSearchBar";

export default function ExplorerBrandsBigram() {
  const nav = useNavigate();
  const [data, setData] = useState<BrandBigramListResponse | null>(null);
  ...
  useEffect(() => {
    fetchApi<BrandBigramListResponse>("/explorer/brands/bigrams", {
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <div className="flex items-baseline gap-4 mb-2">
        <Heading size="6">Brand Bigrams</Heading>
        <Text size="2" style={{ color: "var(--gray-9)" }}>
          Two-token windows extracted from every brand_phrase. Distinctiveness gated on
          the bigram's own corpus IDF — catches cases like "regional group" where
          neither individual token is distinctive.
        </Text>
      </div>

      <div className="flex items-center gap-4 my-5 flex-wrap">
        <TextField.Root size="2" placeholder="Filter bigram…"
        ...
        <table className="w-full text-[13px]">
          <thead className="bg-[var(--gray-2)]">
            <tr style={{ color: "var(--gray-9)" }}>
              <th className="text-left p-2 font-medium">bigram</th>
              ...
              <th className="text-center p-2 font-medium" title="Both constituents are common English/FR">all-EN</th>
              <th className="text-center p-2 font-medium" title="Both constituents are Canadian place names">all-PL</th>
              <th className="text-center p-2 font-medium" title="Both constituents are industry stopwords">all-IND</th>
            </tr>
          </thead>
          <tbody>
            {data?.results.map((t: BrandBigramSummary) => (
              <tr key={t.bigram}
                  className="border-t border-[var(--gray-4)] hover:bg-[var(--gray-2)] cursor-pointer"
                  onClick={() => nav(`/explorer/brands/2gram/${encodeURIComponent(t.bigram)}`)}>
                <td className="p-2 font-mono">{t.bigram}</td>
