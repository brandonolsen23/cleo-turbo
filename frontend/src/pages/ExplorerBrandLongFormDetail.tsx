...
export default function ExplorerBrandLongFormDetail() {
  ...
  const [data, setData] = useState<BrandLongPhraseDetail | null>(null);
  const [err, setErr] = useState<string | null>(null);

  useEffect(() => {
    if (!phrase) return;
    fetchApi<BrandLongPhraseDetail>(`/explorer/brands/long-phrases/${encodeURIComponent(phrase)}`)
  ...
  const tokens = data.phrase.split(" ");

  return (
    <div className="p-6 max-w-6xl mx-auto">
      <Link to="/explorer/brands/long-form" className="text-[13px] no-underline"
            style={{ color: "var(--accent-11)" }}>
        ← 6+ long-form
      </Link>

      <div className="flex items-baseline gap-3 mt-2 mb-1 flex-wrap">
        <Heading size="6" className="font-mono">{data.phrase}</Heading>
        <Badge color={data.is_distinctive ? "jade" : "gray"}>
          {data.is_distinctive ? "distinctive" : "not distinctive"}
        </Badge>
      </div>
      <Text size="2" style={{ color: "var(--gray-9)" }}>
        {data.n_tokens} tokens:{" "}
        {tokens.map((t, i) => (
          <span key={i}><strong>{t}</strong>{i < tokens.length - 1 ? " + " : ""}</span>
        ))}
      </Text>
      ...
      <div className="grid grid-cols-4 gap-4 mt-5">
        ...
        <div className="rounded-[var(--card-radius)] border border-[var(--gray-6)] p-5">
          <Text size="1" style={{ color: "var(--gray-9)" }}>Source phrases</Text>
          <Heading size="5" mt="2">{data.n_distinct_source_phrases.toLocaleString()}</Heading>
        </div>
        <div className="rounded-[var(--card-radius)] border border-[var(--gray-6)] p-5">
          <Text size="1" style={{ color: "var(--gray-9)" }}>n_tokens</Text>
          <Heading size="5" mt="2">{data.n_tokens}</Heading>
        </div>
      </div>
      ...
      <div className="rounded-[var(--card-radius)] border border-[var(--gray-6)] p-5">
        <div className="flex flex-wrap gap-2 items-center">
          ...
          <Text size="2" style={{ color: "var(--gray-9)", marginLeft: "auto" }}>
            distinctiveness is gated on the phrase's own IDF
          </Text>
        </div>
      </div>

      <BrandFamily seed_value={data.phrase} seed_level="long-form" />
