...
export default function ExplorerBrandsTrigram() {
  ...
  useEffect(() => {
    fetchApi<BrandTrigramListResponse>("/explorer/brands/trigrams", {
    ...
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <div className="flex items-baseline gap-4 mb-2">
        ...
        <Text size="2" style={{ color: "var(--gray-9)" }}>
          Three-token windows extracted from every brand_phrase. Most distinct legal
          entity names survive the stopword filter at 3 tokens.
        </Text>
      </div>

      <div className="flex items-center gap-4 my-5 flex-wrap">
        <TextField.Root size="2" placeholder="Filter trigram…"
        ...
        {data && (
          <Text size="2" style={{ color: "var(--gray-9)" }}>
            {data.total.toLocaleString()} trigrams
          </Text>
        )}
      </div>
        ...
        <table className="w-full text-[13px]">
          <thead className="bg-[var(--gray-2)]">
            <tr style={{ color: "var(--gray-9)" }}>
              <th className="text-left p-2 font-medium">trigram</th>
              ...
              <th className="text-center p-2 font-medium">any-tok-dist</th>
              <th className="text-center p-2 font-medium">all-EN</th>
              <th className="text-center p-2 font-medium">all-PL</th>
              <th className="text-center p-2 font-medium">all-IND</th>
            </tr>
          </thead>
          <tbody>
            {data?.results.map((t: BrandTrigramSummary) => (
              <tr key={t.trigram}
                  className="border-t border-[var(--gray-4)] hover:bg-[var(--gray-2)] cursor-pointer"
                  onClick={() => nav(`/explorer/brands/3gram/${encodeURIComponent(t.trigram)}`)}>
                <td className="p-2 font-mono">{t.trigram}</td>
