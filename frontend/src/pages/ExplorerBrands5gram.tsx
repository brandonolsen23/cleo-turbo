...
import type { BrandFivegramListResponse, BrandFivegramSummary } from "../types";
import ExplorerTabs from "../components/explorer/ExplorerTabs";
import BrandSearchBar from "../components/explorer/BrandSearchBar";

export default function ExplorerBrands5gram() {
  const nav = useNavigate();
  const [data, setData] = useState<BrandFivegramListResponse | null>(null);
  ...
  useEffect(() => {
    fetchApi<BrandFivegramListResponse>("/explorer/brands/5grams", {
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <div className="flex items-baseline gap-4 mb-2">
        <Heading size="6">Brand 5-grams</Heading>
        <Text size="2" style={{ color: "var(--gray-9)" }}>
          Five-token windows. Captures government / sovereign / longer institutional names
          where the operator's identity only emerges at 5 tokens.
        </Text>
      </div>

      <div className="flex items-center gap-4 my-5 flex-wrap">
        <TextField.Root size="2" placeholder="Filter 5-gram…"
        ...
        {data && (
          <Text size="2" style={{ color: "var(--gray-9)" }}>
            {data.total.toLocaleString()} 5-grams
          </Text>
        )}
      </div>
        ...
        <table className="w-full text-[13px]">
          <thead className="bg-[var(--gray-2)]">
            <tr style={{ color: "var(--gray-9)" }}>
              <th className="text-left p-2 font-medium">5-gram</th>
          ...
          <tbody>
            {data?.results.map((t: BrandFivegramSummary) => (
              <tr key={t.fivegram}
                  className="border-t border-[var(--gray-4)] hover:bg-[var(--gray-2)] cursor-pointer"
                  onClick={() => nav(`/explorer/brands/5gram/${encodeURIComponent(t.fivegram)}`)}>
                <td className="p-2 font-mono">{t.fivegram}</td>
