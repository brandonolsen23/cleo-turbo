...
import type { BrandBigramDetail } from "../types";
import PartySideCard from "../components/explorer/PartySideCard";
import BrandFamily from "../components/explorer/BrandFamily";

export default function ExplorerBrandBigramDetail() {
  const { bigram } = useParams<{ bigram: string }>();
  const [data, setData] = useState<BrandBigramDetail | null>(null);
  const [err, setErr] = useState<string | null>(null);

  useEffect(() => {
    if (!bigram) return;
    fetchApi<BrandBigramDetail>(`/explorer/brands/bigrams/${encodeURIComponent(bigram)}`)
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      <Link to="/explorer/brands/2gram" className="text-[13px] no-underline"
            style={{ color: "var(--accent-11)" }}>
        ← Bigrams
      </Link>

      <div className="flex items-baseline gap-3 mt-2 mb-1 flex-wrap">
        <Heading size="6" className="font-mono">{data.bigram}</Heading>
        <Badge color={data.is_distinctive ? "jade" : "gray"}>
          {data.is_distinctive ? "distinctive" : "not distinctive"}
        </Badge>
      </div>
      <Text size="2" style={{ color: "var(--gray-9)" }}>
        tokens: <strong>{data.token_a}</strong> + <strong>{data.token_b}</strong>
      </Text>
      ...
      <div className="rounded-[var(--card-radius)] border border-[var(--gray-6)] p-5">
        <div className="flex flex-wrap gap-2 items-center">
          ...
          <Text size="2" style={{ color: "var(--gray-9)", marginLeft: "auto" }}>
            distinctiveness is gated on the bigram's own IDF
          </Text>
        </div>
      </div>

      <BrandFamily seed_value={data.bigram} seed_level="2gram" />

      <Heading size="4" mt="6" mb="2">Brand phrases containing this bigram</Heading>
