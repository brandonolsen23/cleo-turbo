...
export default function GroupsPage() {
  ...
  const sort = searchParams.get("sort") || "transaction_count";
  ...
  const minRadiusKm = searchParams.get("min_radius_km") || "";
  const maxRadiusKm = searchParams.get("max_radius_km") || "";
  ...
  const minBrandCount = searchParams.get("min_brand_count") || "";
  ...
  const fetchData = useCallback(() => {
    ...
    if (minRadiusKm) params.min_radius_km = minRadiusKm;
    ...
    if (minBrandCount) params.min_brand_count = minBrandCount;
    ...
    minNetAcquisitions, maxNetAcquisitions, minRadiusKm, maxRadiusKm,
    assetClass, minAssetClassCount, maxAssetClassCount, brand, minBrandCount, region,
  ]);
  ...
  // URL update helper — sets param and resets to page 1
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      >
        ...
        <RangeFilter
          label="Geo Radius (km)"
          minValue={minRadiusKm}
          maxValue={maxRadiusKm}
          onMinChange={(v) => setParam("min_radius_km", v)}
          onMaxChange={(v) => setParam("max_radius_km", v)}
          placeholder={["Min", "Max"]}
        />
        <SelectFilter
          label="Region"
        ...
        <ComboFilter
          label="Tenant Brand"
          selectValue={brand}
          numberValue={minBrandCount}
          onSelectChange={(v) => {
            setParam("brand", v);
            if (!v) setParam("min_brand_count", "");
          }}
          onNumberChange={(v) => setParam("min_brand_count", v)}
          options={(filterOptions?.brands || []).map((b) => ({
            value: b,
            label: b,
          }))}
          selectPlaceholder="Any brand"
            ...
            {data?.results.map((g) => (
              ...
              >
                ...
                <td className="px-4 py-2">
                  <Badge
                    size="1"
                    color={g.status === "engaged" ? "jade" : "gray"}
                    variant={g.status === "engaged" ? "solid" : "soft"}
                  >
                    {g.status}
                  </Badge>
                </td>
